...
import { Scenario } from './vo/scenario';
import { GraphData } from './vo/graphData';
import { MCData } from './vo/mcData';
import { PortfolioComposition } from './vo/portfolioComposition';
import { CreditSummary } from './vo/creditSummary';
import { Loan } from './vo/loan';
import { UploadedPortfolio } from './vo/uploadedPortfolio';
import { LoanDetail } from "./vo/loanDetail";
...
function getSectors(userID): Array<Sector> {
    console.log("server:  getSectors for user id: "+userID);
    return sectors;
}


function getScenarios(userID): Array<Scenario> {
    console.log("server:  getScenarios for user id: "+userID);
    return scenarios;
}


function getGraphs(userID): Array<Scenario> {
...
function getPortfolioComposition(userID): Object {
    console.log("server:  getPortfolioComposition for user id: "+userID);
    return portfolioCompositions[0];
}


function getCreditSummary(userID): Array<Scenario> {
    console.log("server:  getCreditSummary for user id: "+userID);
    return creditSummary;
}


function get1YSummary(userID): Array<Scenario> {
    console.log("server:  getCreditSummary for user id: "+userID);
    return creditSummary;
}


function getLoans(userID): Array<Scenario> {
...
function getUploadedPortfolios(userID): Array<Portfolio> {
...
//-----
app.get('/api/upPortfolios/:userID', (req, res) => {  //OK: was: /portfolios/:id
...
app.get('/api/loandetails/:userID/:portfolioName/:sector/:propId/:shared/:type/:term', (req, res) => {  //OK: was: /portfolios/:id
...
app.get('/api/portfolios/:userID', (req, res) => {  //OK: was: /portfolios/:id
...
app.get('/api/sectors/:portfolioName/:userID/:scenario/:shared', (req, res) => {  //OK: was: /portfolios/:id
...
app.get('/api/scenarios/:portfolioName/:userID/:shared', (req, res) => {  //OK: was: /portfolios/:id
...
app.get('/api/graphs/:portfolioName/:userID/:sector/:term/:shared', (req, res) => {  //OK: was: /portfolios/:id
...
app.get('/api/mc/:portfolioName/:userID/:sector/:scenarioName/:interval/:shared', (req, res) => {  //OK:
...
app.get('/api/portfolioComposition/:portfolioName/:userID/:sector/:shared', (req, res) => {
...
app.get('/api/summary/:portfolioName/:userID/:sector/:term/:shared', (req, res) => {  //OK: get credit summary
...
app.get('/api/summary1y/:portfolioName/:userID/:sector/:term/:shared', (req, res) => {  //OK: get 1yr summary
...
app.get('/api/loans/:userID/:portfolioName/:sector/:scenario/:sort/:shared', (req, res) => {  //OK: was: /portfolios/:id
...
const server = app.listen(8000, "0.0.0.0", () => {   //OK: was localhost
...
var sql
var request;
var config = {  //"mssql://mbscre_user:mbs123@REISMBSDEV/MBSCRE_DW"
    user: 'mbscre_user',
    password: 'mbs123',
    server: 'REISMBSDEV',  //'localhost', // You can use 'localhost\\instance' to connect to named instance
    database: 'MBSCRE_DW',
...
function getSectorsFromDB (response, portfolioName, userID, scenario, shared) {
    ...
    request.on('row', function (row) {
        ...
        var sector : Sector = new Sector(
            row.fullName,
            getDollarNumWithNull(row.upb),
            getPercentWithNull(row.upbPerc),
            row.totalLoan,
            row.osSector);
        sectors.push(sector);
...
function getGraphDataFromDB (response, portfolioName, userID, sector, term, shared) { // get Graph data from DB
    graphs = [];
    ...
    request.on('row', function (row) {
        ...
        var graph : GraphData = new GraphData (
            row.scenario_id,
            row.scenario_name,
            row.pd, //getNumWithNull2(row.pd),  //row.pd.toFixed(2),
            row.perc,
            row.month,
            row.label,
            row.el, //getNumWithNull2(row.el),  //row.el.toFixed(2),
...
function getMCDataFromDB (response, portfolioName, userID, sector, scenarioName, interval, shared) { // get MonteCarlo (grid) data from DB
    mcdata = [];
    ...
    request.on('row', function (row) {
        // Emitted for each row in a recordset
        //console.log("raw row: "+JSON.stringify(row));
        var mc : MCData = new MCData (
            row.year_month,
            row.pd_cum*100, // getPercentWithNull(row.pd_cum*100),
...
function getPortfolioCompositionFromDB(response, portfolioName, userID, sector, shared) {
    ...
    request.on('row', function (row) {
        ...
        // Emitted for each row in a recordset
        var portfolioComposition: PortfolioComposition = new PortfolioComposition (
            row.loan_count,
            getDollarNumWithNull(row.original_upb),
            getDollarNumWithNull(row.current_upb),
            getNumWithNull2(row.wa_dscr),
            getPercentWithNull(row.wa_ltv*100),
            getPercentWithNull(row.wa_debtYield*100),
            getPercentWithNull(row.wa_coupon),
            getDollarNumWithNull(row.avg_loan_size),
            getDollarNumWithNull(row.largest_loan),
            getNumWithNull2(row.wa_rem_term),
            getNumWithNull2(row.wa_rem_amort),
            getNumWithNull2(row.wa_loan_age),
            getPercentWithNull(row.perc_ballon),
            getPercentWithNull(row.interest_only),
            getPercentWithNull(row.perc_fixed),
            row.curr_y_qtr,
            row.base_year,
            row.observation_dateDB);
    ...
    request.on('done', function (affected) {    //OK: should be only 1 row
        // Always emitted as the last one
...
function getCreditSummaryFromDB (response, portfolioName, userID, sector, term, shared) {
    ...
    request.on('row', function (row) {
        // Emitted for each row in a recordset
        //console.log("raw row: "+JSON.stringify(row));
        var summaryRow : CreditSummary = new CreditSummary (
            row.scenario,
            row.scenario_name,
            getPercentWithNull(row.pd*100),
            getPercentWithNull(row.ls*100),
            getPercentWithNull(row.el*100),
            row.perc,
            getDollarNumWithNull(row.value),
            getPercentWithNull(row.yield_degradation) );
...
function getLoansFromDB (response, userID, portfolioName, sector, scenario, sort, shared) {
    ...
    request.on('row', function (row) {
        ...
        var loan : Loan = new Loan (
            ...
            row.subname,
            row.sector,
            getDollarNumWithNull(row.current_balance),
            getPercentWithNull(row.pd_cum),
            getPercentWithNull(row.ls_cum),
            getPercentWithNull(row.el_cum),
            getDollarNumWithNull(row.el_value),
            getNumWithNull2(row.dscr),
            getPercentWithNull(row.current_ltv));
...
function getUploadedPortfoliosFromDB (response, userID) {
    var upPortfolios:Array<Portfolio> = [];  // array of Uploaded Portfolios
    ...
    sql.connect(config, function (err) {
        ...
        request.on('row', function (row) {
            // Emitted for each row in a recordset
            var upPortfolio = new UploadedPortfolio (
                row.portfolio_dim_id,
                row.portfolioName,
                getDollarNumWithNull(row.upbDollars),
                row.portfolioUpdated,
                ...
                row.loanCount,
                row.status,
                row.propertyUniqueFileName,
                row.creditUniqueFileName);
...
function getLoanDetailsFromDB (response, userID, portfolioName, sector, propId, shared, type, term) {
    ...
    request.on('row', function (row) {
        // Emitted for each row in a recordset
        //console.log("raw row: "+JSON.stringify(row));
        var loanDetail : LoanDetail = new LoanDetail (
            row.loan_id_client,
            row.property_id_client,
            getDollarNumWithNull(row.original_balance),
            getDollarNumWithNull(row.current_balance),
            row.scenario_name,  // + status?
            row.loan_origin_date,
            row.maturity_date_orig,
            row.loan_type,
            row.margin,
            row.interest_rate,
            row.reset_freq,
            row.next_reset_date,
            row.amort_type,
            row.amort_term_months,
            row.amort_term_remain,
            row.remaining_term,
            row.balloon,
            row.io_term,
            row.loss_share_pct,
            row.loan_risk_rating,
            row.client_risk_rating,
            row.debtYield,
            row.yieldDegradation,
